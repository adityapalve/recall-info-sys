...
export function Home({ content, settings, notice, onStart, onSettings }: Props) {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden px-6 pt-3 pb-4">
      ...
      <div className="shrink-0 bg-zinc-950 pt-3">
        {failure ? (
        ...
        {notice ? (
          <p className="mb-3 rounded-xl bg-zinc-900 p-3 text-center text-sm text-zinc-300">
            {notice}
          </p>
        ) : null}
        {/*<p className="mb-2 text-sm text-zinc-500">Session size</p>*/}
        <div className="mb-3 grid grid-cols-3 gap-2 rounded-xl bg-zinc-900 p-1">
          {SIZES.map((n) => (
            <button
              key={n}
              type="button"
